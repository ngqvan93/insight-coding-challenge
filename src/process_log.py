...
def make_feature_1(df, output_path):
	...
	top_host = df['host'].value_counts()[0:10]
	top_host.to_csv(path = output_path, sep = ',')
...
def make_feature_3(df, output_path):
	...
	busiest_time = df.sort_values('sum', ascending = False)[0:10][['time_full', 'sum']]
	busiest_time.to_csv(path_or_buf = output_path, sep = ',', index = False, header = False)
